...
public class ControllerOld {
    ...
    private void attemptMove(BoardModel board, SquareView view, SquareModel model) {
        ...
        PieceColor targetPieceColor = null;
        
        if(model.getPiece() != null){
            targetPieceColor = model.getPiece().getColor();    
        }


        if(targetPieceColor == currentColor()){
            System.out.println("Target color is gelijk aan huidige kleur ");
           // stage = SelectionStage.NONE;
            ...
            stage = SelectionStage.SOURCE_SELECTED;
            System.out.println( stage +" " +model.getPiece() + model.getPosition());
            return;
        }
        ...
        if (valid) {
            ...
            System.out.println(" move invalid and reset selection and view remove selection");
            System.out.println("source= " +sourceView+  " selectedPiece= " + selectedPiece +  " view pos= "  + sourcePos
            + " lastmove " +lastmove);
            sourceView.removeSelection();
            view.removeSelection();
            resetSelection();
